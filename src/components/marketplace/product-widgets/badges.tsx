...
// Temporary until we fully migrate Chip or create a custom Badge component
// For now, we'll use a simple Tailwind implementation to avoid MUI dependency if possible,
// but the instruction was to Replace MUI. So let's use tailwind directly.
...
const getColorClass = (color: string | undefined, variant: 'filled' | 'outlined') => {
  const isFilled = variant === 'filled';
  switch (color) {
    case 'primary': return isFilled ? 'bg-primary text-white' : 'border-primary text-primary';
    case 'secondary': return isFilled ? 'bg-secondary text-white' : 'border-secondary text-secondary';
    case 'success': return isFilled ? 'bg-green-600 text-white' : 'border-green-600 text-green-600';
    case 'warning': return isFilled ? 'bg-yellow-500 text-white' : 'border-yellow-500 text-yellow-500';
    case 'info': return isFilled ? 'bg-blue-500 text-white' : 'border-blue-500 text-blue-500';
    case 'error': return isFilled ? 'bg-red-500 text-white' : 'border-red-500 text-red-500';
    default: return isFilled ? 'bg-gray-200 text-gray-800' : 'border-gray-200 text-gray-800';
  }
};

export const PromoBadges = ({ label, color = 'default' }: BadgeProps) => {
  const colorClass = getColorClass(color, 'filled');
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded text-[0.6em] font-medium ${colorClass}`}>
      {label}
    </span>
  );
};

export const VerifiedBadges = ({ label, color = 'default', className }: BadgeProps) => {
  const colorClass = getColorClass(color, 'outlined');
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded border-2 text-[0.8em] font-medium h-6 gap-1 ${colorClass} ${className}`}>
...
export const NormalBadges = ({ label, color = 'default', className }: BadgeProps) => {
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded text-[0.8em] bg-[#16b364] text-white h-8 gap-1 ${className}`}>
      <GiMining className="w-4 h-4" />
      {label}
    </span>
  );
};

export const CertificateBadge = ({ src }: { src: string }) => {
  return (
    <div>
      <img src={src} alt="Certificate" className="w-[15px]" />
    </div>
  );
};

export const PricingBadge = ({ fill }: { fill: string }) => {
  return (
    <div>
      <svg fill="#000000" height="20px" width="20px" viewBox="0 0 59.986 59.986">
        <path
          fill={fill}
          d="M59.972,20.164c0.007-0.041,0.011-0.081,0.013-0.123c0.002-0.049,0.001-0.096-0.003-0.145
