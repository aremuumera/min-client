            ...
            {fields?.length > 0 && fields?.map((field: any) => (
                  <Box key={field.id} className="mb-2">
                    ...
                    {field.type === 'select' && (
                      <Select
                        fullWidth
                        // multiple={field.multipleFields as boolean} // Enable multiple selections
                        value={field.multipleFields ? formData[field.id] || [] : formData[field.id] || ''} // Ensure value is an array for multiple fields
                        onChange={(e: any) => handleChange(field.id, e.target.value)}
                      ...
                      <Box>
                        ...
                        <Typography variant="body2" className="pt-3 text-center mb-2">
                         ...
                         <Box                            
                               ...
                               className="!flex  relative space-x-2">
                                {attachments?.length > 0 && attachments?.map((img: any, i: number) => (
                                  ...
                                  className="w-40 h-32 o rounded flex-shrink-0"
                                  key={i}>
