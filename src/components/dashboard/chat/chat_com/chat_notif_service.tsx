...
class ChatApiService {
  ...
  async sendBatchChatEmailNotifications(batchData: any) {
    try {
      const response: any = await AxiosInstance.post(`${API_HOSTNAME}/notifications/chat/batch`, batchData);

      if (!response?.data?.status || response?.data?.status !== 'success' || response?.status !== 'success') {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
